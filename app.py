...
with tabs[8]:
    ...
    if st.button("Generate & Download"):
        # 1. Prepare all non-empty stored data
        non_empty = {sec: df for sec, df in st.session_state.store.items() if not df.empty}

        # 2. Add "Individual" sheet manually if ID is filled
        if ind_id:
            individual_df = pd.DataFrame([{
                "Individual ID": ind_id,
                "Age (days)": ind_age,
                "Sex": ind_sex,
                "Karyotypic Sex": ind_ksex,
                "Vital Status": ind_vital,
                "Last Encounter": ind_last.strftime("%H:%M")
            }])
            non_empty["Individual"] = individual_df
